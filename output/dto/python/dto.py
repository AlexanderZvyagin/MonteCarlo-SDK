...
# Forward declaration
class CashFlows: pass
class CashFlows (Updater):
...
'''
    ...
    def __neq__ (self, other):
        return not self==other
    def json (self) -> str:
        return CashFlows_to_json_string(self)
def CashFlows_from_json_string (jstr):
    j = json.loads(jstr)
    obj = CashFlows()
    CashFlows_from_json(j,obj)
    return obj

def CashFlows_to_json_string (self:CashFlows):
    j = {}
    CashFlows_to_json(j,self)
    return json.dumps(j)
def CashFlows_from_json (j:dict, obj:CashFlows):
    assert isinstance(obj,CashFlows)
    Updater_from_json(j,obj)
def CashFlows_to_json(j:dict, obj:CashFlows):
    Updater_to_json(j,obj)
    pass
