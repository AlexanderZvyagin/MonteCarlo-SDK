...
public:
    ...
    std::optional<std::vector<float>> start;
    std::optional<std::string> logging;
...
void to_json(json &j, const UpdaterDto &obj) try {
    ...
    if(obj.logging.has_value())
        j["logging"] = obj.logging.value();
...
void from_json(const json &j, UpdaterDto &obj) try {
    ...
    if(auto it=j.find("start"); it!=j.end() and !it->is_null())
        obj.start = *it;
    if(auto it=j.find("logging"); it!=j.end() and !it->is_null())
        obj.logging = *it;
} catch (const std::exception &e) {
