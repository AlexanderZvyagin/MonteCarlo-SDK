...
class DtoError:
    ...
    def __init__ (
        ...
        code:int|None = None,
        errors:list[DtoError]|None = None
    ):
        self.message : str|None = message
        self.details : str|None = details
        self.code : int|None = code
        self.errors : list[DtoError]|None = deepcopy(errors)
        pass
    ...
    def json (self) -> str:
        return DtoError_to_json_string(self)
def DtoError_from_json_string (jstr):
    j = json.loads(jstr)
    obj = DtoError()
    DtoError_from_json(j,obj)
    return obj

def DtoError_to_json_string (self:DtoError):
    j = {}
    DtoError_to_json(j,self)
    return json.dumps(j)
def DtoError_from_json (j:dict, obj:DtoError):
    assert isinstance(obj,DtoError)
    ...
    if j.get("errors",None) is not None:
        ...
        for item in j["errors"]:
            v = DtoError()
            DtoError_from_json(item,v)
            obj.errors.append(v)
def DtoError_to_json(j:dict, obj:DtoError):
